...
Open **Desk Background Settings** from the **UI Styles** workspace (System Manager).

| Field | fieldname | Purpose |
| --- | --- | --- |
| *Enable Desk Background* | `enabled` | Master switch. Default after install: off. |
| *Site Colour Preset* | `site_color_preset` | Site-wide colour. **Standard** removes the tint. Default when enabled: **Standard**. |
| *Colour Strength* | `site_color_intensity` | **Tint** (softer) or **Strong** (more vivid). Default: **Tint**. Hidden when preset is **Standard**. |
| *Allow users to choose their own colour* | `allow_user_color` | When enabled, each user may set *Desk Colour* and *Desk Colour Strength* on their **User** record. Default: off. |
...
**Light Red** + **Tint** matches the legacy soft red test-system look from earlier standalone app versions.
...
See also: [Personal desk colour](/app/docs/en/ui-styles/desk-background/user-settings).
