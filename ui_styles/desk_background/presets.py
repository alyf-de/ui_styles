...
# Frappe Desk text colours used only in tests to assert WCAG AA contrast of presets.
DARK_TEXT = "#ededed"  # light text on dark surfaces (dark mode)
LIGHT_TEXT = "#171717"  # dark text on light surfaces (light mode)
...
def relative_luminance(hex_color: str) -> float:
	...
	Used by tests (and ``contrast_ratio``) so preset surfaces stay readable;
	not used at runtime when applying Desk tints.
	"""
...
def contrast_ratio(color1: str, color2: str) -> float:
	...
	Tests require at least 4.5:1 (AA) between each preset surface/navbar and
	Desk text (``LIGHT_TEXT`` / ``DARK_TEXT``). Not used when applying tints.
	"""
