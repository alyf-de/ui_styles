...
from ui_styles.desk_background.presets import DEFAULT_INTENSITY, STANDARD
...
def clear_all_user_presets():
	...
	preset = getattr(user, USER_CUSTOM_FIELD)
	intensity = getattr(user, USER_INTENSITY_FIELD)

	(
		frappe.qb.update(user)
		.set(preset, "")
		.set(intensity, "")
		.where(Criterion.any([IfNull(preset, "") != "", IfNull(intensity, "") != ""]))
		.run()
	)
